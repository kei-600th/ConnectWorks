  ...
  <div>
    <div class="status-container">
      ...
      >
        ...
        <v-list class="rounded-list">
          <div v-for="(task, taskIndex) in status.tasks" :key="taskIndex">
            <v-card
            ...
            <v-card v-if="task.isNew" color="#385F73" class="styled-card">
              <v-card-subtitle>
                <v-text-field v-model="task.name" label="新規のタスク">
                </v-text-field>
              </v-card-subtitle>
              <v-card-actions>
    ...
    <v-dialog v-model="dialog" width="500" :persistent="persistent">
      <v-card v-if="!isEditing">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="editModal(showTask)">
            編集
          </v-btn>
          <v-btn color="error" text @click="deleteTask()"> 削除 </v-btn>
          <v-btn color="primary" text @click="dialog = false"> 戻る </v-btn>
        </v-card-actions>
      </v-card>
      <v-card v-if="isEditing">
        <v-card-title>
          <v-text-field v-model="editTask.name" label="タスク名">
          </v-text-field>
        </v-card-title>
        <v-card-text>
          <v-select v-model="editTask.status" :items="statusNames"> </v-select>
        </v-card-text>
        <v-card-text>
          <v-textarea v-model="editTask.describe"> </v-textarea>
        </v-card-text>
        <v-divider></v-divider>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="updateTask(editTask)">
            保存
          </v-btn>
          <v-btn color="primary" text @click="cancelEditModal()"> 取消 </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    async taskCreate(submitTask) {
      if (submitTask.name !== '') {
        try {
          ...
          const deleteStatusIndex = this.statuses.findIndex(
            (item) => item.name === submitTask.status
          )
    ...
    async updateTask(submitTask) {
      if (submitTask.name !== '') {
        try {
          ...
          this.splice()
          const addIndex = this.statuses.findIndex(
            (item) => item.name === submitTask.status
          )
    ...
    splice() {
      const deleteStatusIndex = this.statuses.findIndex(
        (item) => item.name === this.showTask.status
      )
      const tasksArray = this.statuses[deleteStatusIndex].tasks
      const taskIndex = tasksArray.findIndex(
        (task) => task.id === this.showTask.id
      )
      if (taskIndex !== -1) {
